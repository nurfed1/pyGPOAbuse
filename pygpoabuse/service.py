...
class Service:
    def __init__(self, service_name, mod_date="", old_value=""):
        if mod_date:
        ...
        self._file_str = f"""<NTService clsid="{{AB6F0B67-341F-4e51-92F9-005FBFBA1A43}}" name="{service_name}" image="0" changed="{self._mod_date}" uid="{{{self._guid}}}" userContext="0" removePolicy="0"><Properties startupType="NOCHANGE" serviceName="{service_name}" serviceAction="RESTART" timeout="30"/><Filters><FilterRunOnce hidden="1" not="0" bool="AND" id="{{47511D93-4E18-4680-AFBA-3F463BC48C4C}}"/></Filters></NTService>"""
